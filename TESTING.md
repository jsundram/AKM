...
```
node archive/parser-test.js     # 72/72 — grid parsing + multi-user matching + faculty coaching view + informational events (everyone), synthetic fixture (offline; runs anywhere)
...
node scripts/dress-test.js      # 7/7 — dress cards == the day's concert cast (Beethoven-phantom + Haydn-drop guards) + week-aware repertoire cross-check, live
```
