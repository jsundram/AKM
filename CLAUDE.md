...
- `roster.html` — standalone **noindex** participant roster (footer-linked; "← Schedule" back). Pulls its *own* view-only Google Sheet via gviz JSONP, caches it in `localStorage` (`akm-roster`), offline-first like the schedule, and re-pulls on foreground. Set `ROSTER_SID` / `ROSTER_GID` at the top (`ROSTER_GID` currently points at the cleaned tab where one-person-per-row split the old shared-room rows). The **sheet** carries `# · Name · Instrument · Type · Hotel · Pieces · Notes · Hometown · WhatsApp`; the **displayed table** is the PDF-style hotel-tinted `# · Name · Instr. · Type · Hotel` (hotel→fill palette inline, mirroring the PDF tool). **Columns map by header name** (`colMap`), so extra/renamed/reordered columns are safe. **Every visible column header sorts** (toggle asc/desc): `#` numeric (handles half-rows like `17.5`), **Type** by role rank `DIR→MGR→F→AF→TF→W1→W2→G` (**G** = non-playing guests, e.g. family sharing a room), **Instr.** collapsed + ordered to `V, V/VA, VA, VC, Bass, Piano, Clar.` (`instLabel`/`INST_ORDER`: all violin parts→V, VA1→VA, V/VA kept distinct before VA), others lexical, blanks last. Rows with **From / Pieces / Notes tap to expand** (From = Hometown; URLs in Notes auto-link); a **WhatsApp glyph** on rows that have a number opens `wa.me/<digits>?text=…` pre-filled (`WA_MSG`); Jason's own row gets the brass "you" marker. Footer also links **Network** and **Edit sheet ↗** (deep-links to the tab via `ROSTER_GID`). **Roster data is deliberately not in the repo** (PII) — edit the sheet to update.
