...
- `app.js` — the brain: fetch → parse → render → cache. Ported 1:1 from the earlier Python runner. Also makes a side pull of the **roster sheet** (`RSID`/`RGID`) to map each person's first name → Notes-URL (`coachUrls`, cached in `c.coaches`), so a **coach's name in a rehearsal/lesson card links to their bio** (`coachLink`; first-name match — faculty URL-holders don't collide; underlined in muted ink, *not* the reserved brass).
