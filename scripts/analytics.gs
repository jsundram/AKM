...
//      Put headers in row 1: received · opened · page · who · action · to · label
...
//
// To add the action/to/label columns to an already-live sheet: add the three headers, Plain-text
// the `to` column, re-paste this file, and redeploy (New version) so doGet writes the extra cells.
//
...
// event's target (the kudos recipient), reversible with =UID() just like who; label = optional
// non-PII plaintext (the kudos composer), "" otherwise.
...
function doGet(e){
  ...
  if (p.k === TOK)
    SpreadsheetApp.getActive().getSheets()[0]
      .appendRow([new Date(), p.ts ? new Date(+p.ts) : "", p.p || "", p.u || "", p.a || "", p.t || "", p.l || ""]);
